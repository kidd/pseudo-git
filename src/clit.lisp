(in-package :cl-user)
;; * doc
;; BEN HOYTHome Resume/CV Projects Writings Contact
...
;; Somewhat inspired by Mary Rose Cook’s similar effort, I wanted to see if I could implement enough of git to create a repo, perform a commit, and push to a real server (GitHub in this case).
...
;; status uses get_status() to compare the files in the index to the files in the current directory tree, and prints out which files are modified, new, and deleted
...
;; pygit is also a pretty decent example of using the standard library argparse module, including sub-commands (pygit init, pygit commit, etc). I won’t copy the code here, but take a look at the argparse code in the source.
...
(defpackage clit
  (:use :cl)
  ;; (:use :ironclad :arnesi :salza2 :pack)
  )
...
(defun unpack-index-file (path)
  (with-open-file (s path
                     :direction :input
                     :if-does-not-exist :create
                     :element-type '(unsigned-byte 8))
    (format t "~a"
            (pack:unpack "!LLLLLLLLLL20sH" s))
    ;; (format t
    ;;         "~a"
    ;;         (pack::read-u2 s )
    ;;         ;;(read-byte s nil 'eof)
...
(defun ff (path)
  (alexandria:with-input-from-file (s path :element-type '(unsigned-byte 8))
    (pack:unpack "!4sLL" s)))
...
(defun get-path (stream)
  (let ((path (format nil
    ...
    (loop for i from 1 upto 7
          until (= 0 (rem (+ 62 (length path) i) 8))
          do (read-byte stream))
    path))

(defun read-index (f-path)
  (let* ((fields '(ctime_s ctime_n mtime_s mtime_n
                   dev ino mode uid gid size sha1 flags))
         (contents (alexandria:read-file-into-byte-vector f-path))
         (len (length contents))
         (data (subseq contents 0 (- len 20)))
         (sha (subseq contents (- len 20))))
    (alexandria:with-input-from-file (s f-path :element-type '(unsigned-byte 8))
      (loop with (dirc version num-entries) = (pack:unpack "!4sLL" s)
            with _dirc = (assert (string= "DIRC" dirc))
            with _vers = (assert (= 2 version))
            for i from 1 upto num-entries
            for (ctime_s
